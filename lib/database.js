...
// Delete a user from the database based on their email and add them to
// the blacklist
...
exports.isBanned = (email, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM blacklist WHERE email = $1', [email],
      (err, data) => {
        if (err) {
          cb('Unable to connect to the database: ' + err);
          return;
...
exports.getClassList = (cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM classes ORDER BY classid ASC', (err, data) => {
      done();
...
exports.getSubjectList = (cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM subjects ORDER BY subject ASC', (err, data) => {
      done();
...
exports.getPostsByClass = (classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT posts.uid, posts.pdate, posts.num, posts.pid, ' +
            'posts.pcontent FROM posts WHERE num = $1';
    client.query(q, [classid], (err, data) => {
      done();
      ...
      cb(undefined, (data.rows).reverse());
    });      
  });
};
...
exports.createPost = (uid, title, content, classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    if (err) {
      cb('Unable to connect to database: ' + err);
      return;
    ...
    var q = 'INSERT INTO posts VALUES ($1, NOW(), $2, $3, $4)';
    client.query(q, [uid, classid, title, content], (err, data) => {
      done();
...
exports.getEventDetails = (classid, eid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM events WHERE classid = $1 AND eid = $2', 
      [classid, eid], (err, data) => {
      done();
...
//returns all events for a specific class
exports.getEventsByClass = (classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    //gets events from the next 14 days
    var q = 'SELECT *  FROM events WHERE classid = $1 AND DATE > current_date - 1 AND DATE < current_date + 14'
    client.query(q, [classid], (err, data) => {
      done();
...
=======
>>>>>>> b13fd2f0205185aa3799bba06ce89281b31f5741
