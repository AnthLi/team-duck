...
exports.getUserClasses = (cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM "public"."classes"', (err, data2) => {
      done();
...
exports.getClassDetails = (class_id, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT classes.num, students.spireid, users.fname FROM classes ' +
            'JOIN students ON class_id = $1 JOIN users ON users.spireid = ' +
            '(select spireid from users where students.spireid = users.spireid)'
...
exports.addStudentClass = (class_id, num, spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    if (err) {
      cb('Unable to connect to database: ' + err);
      return;
    ...
    client.query(q , [spireid, class_id, num, today], (err, data) => {
      done();
