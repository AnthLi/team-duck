...
exports.updateAbout = (about, spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('UPDATE users SET about = $1 where spireid = $2', [about], [spireid], 
      (err, data) => {
      done(); // Release the client back to the pool
...
exports.getProfile = (spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM users where spireid = $1', [spireid], (err, data) => {
      done();
...
exports.getStudents = (cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('select spireid from users where spireid IS NOT null', (err, students) => {
      done();
