...
exports.addStudentClass = (spireid, class_id, num, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'INSERT INTO students (spireid, class_id, num, date) SELECT ' + 
            'CAST($1 AS VARCHAR), $2, CAST($3 as VARCHAR), $4 WHERE NOT ' + 
            'EXISTS (SELECT spireid, num FROM students WHERE spireid = $1' + 
            'AND num = $3)';
    client.query(q , [spireid, class_id, num, date], (err, data) => {
      done();
