...
exports.getPostsByClass = (classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT * FROM posts WHERE classid = $1';
...
//mkaes a post for user
exports.createPost = (spireid, title, content, classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    if (err) {
      cb('Unable to connect to database: ' + err);
      return;
    ...
    client.query(q, [spireid, classid, title, content], (err, data) => {
      done();
