...
exports.addStudentClass = (num, spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    // Add the spireid to the column if it does not already exist
    var q = 'UPDATE classes SET students = CONCAT(students, CAST($2 AS ' +
            'VARCHAR)) WHERE num = $1 AND NOT EXISTS (SELECT students FROM ' +
            'classes WHERE students LIKE \%$2\%)';
...
exports.getPersonalClasses = (spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    client.query('SELECT * FROM classes WHERE students LIKE \'%$1\'', [spireid], 
      (err, data) => {
      done();
