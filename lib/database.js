...
exports.createEvent = (name, title, description, location, date, classid, spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    if (err) {
      cb('Unable to connect to database: ' + err);
      return;
    ...
    client.query(q, [name, title, description, location, date, classid, spireid], (err, data) => {
      done();
...
exports.updateAttending = (classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'UPDATE schedule SET attending = attending + 1 WHERE classid = $1'
    client.query(q, classid, (err, data) => {
      done();
...
exports.getEventsByClass = (classid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT *  FROM events WHERE classid = $1'
    client.query(q, [classid], (err, data) => {
      done();
