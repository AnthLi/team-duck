...
exports.getPersonalClasses = (spireid, cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT * FROM classes WHERE $1 = ANY (students)';
    client.query(q, [spireid], (err, data) => {
      done();
...
exports.postComment = (uid, pid, cid, content, cb) => {
  pg.connect(connstr, (err, client, done) => {
    if(err){
      cb('Unable to connect to database: ' + err);
      return;
    }
    q = 'INSERT INTO comments VALUES ($1, $2, $3, $4 ,NOW())';
    client.query(q, [uid, pid, cid, content], (err, data) => {
      done();
      if(err){
         cb('Unable to connect to database: ' + err);
        return;
