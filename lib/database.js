...
exports.createEvent = (name, title, description, location, date, classid,  cb) => {
  pg.connect(connstr, (err, client, done) => {
    if (err) {
      cb('Unable to connect to database: ' + err);
      return;
    ...
    var q = 'INSERT INTO events VALUES ($1, $2, $3, $4, $5, $6)';
    client.query(q, [name, title, description, location, date, classid], (err, data) => {
      done();
