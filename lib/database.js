...
exports.users = (cb) => {
  pg.connect(connstr, (err, client, done) => {
    ...
    var q = 'SELECT * FROM users ORDER BY flags DESC';
...
exports.getCID = (pid, cb) => {
  ...
  exports.incrementFlag = (spireid, cb) => {
    ...
    pg.connect(connstr, (err, client, done) => {
      ...
      var q = 'UPDATE users SET flags=flags+1 WHERE spireid=$1';
      client.query(q, [spireid], (err, data) => {
        done();
...
//     var q = 'SELECT * FROM posts WHERE spireid=$1 AND ARRAY_LENGTH(flag,1) > 3';
//     client.query(q, [spireid], (err, data) => {
//       done();
...
//       var q2 = 'SELECT * FROM comments WHERE spireid=$1 AND ARRAY_LENGTH(flag, 1) > 3';
//       client.query(q2, [spireid], (err, data2) => {
//         done();
...
//         cb(undefined, data.length + data2.length);
